<template>         
  <div class="content Coef_scale12 card-rows " >
    <div v-for="Card in Cards" :key="Card.id" :class="getClassCard(Card)">
      ...
      <!-- is a card -->
        <!-- type of card -->
        <div class="full-card-template" :style='"background-image: url(" + getHost() + "/img/Templates-card-type/" + getCardTypeById(Card).png + ".png);"' >
        </div>
        <!-- Card Image -->
        <div class="full-card-art" :style='"background-size: 287px 209px; background-image: url(" + Card.artwork + "); top:10%;"' >
        ...
        <div class="bottom-bar-full" style="width:239.75%;bottom:-70px;">
          <div class="cost-container-full">
            <div class="coin-cost-full" v-if="getCardCost(Card).treasure > 0 || ( getCardCost(Card).potion + getCardCost(Card).debt == 0)">
              <div class="coin-cost-full-text" style="top:12px;">{{ getCardCost(Card).treasure }}<sup v-if="DoNeedSub(Card)" class="supetoile"/></div>
...
export default class CardOnlinePageComponent extends Vue {
  ...
  //console.log(this.set.otherCards);
  ...
  DoNeedSub(currentCard: DigitalCard) {
  ...
  getCardTypeLenght(currentCard: DigitalCard,) {
  getSplitLabel(label: string, index: number) {
  ...
  ***************************************************/
  ...
  getCardTypeById(currentCard: DigitalCard):DisplayableCardType {
    let card;
    card = DominionSets.getCardById(currentCard.id);
    if (card.constructor.name == this.getSupplyCard()) {
      let extension;
      extension ="";
  ...
  getCardSetYear(currentCard: DigitalCard) {
    let CardSetid = this.set.setId
    if ((CardSetid as string) === "undefined") { CardSetid= this.getCardSetById(currentCard); }

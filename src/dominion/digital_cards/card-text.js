...
webclient.component('cardTextBlock', {
    ...
    controller: function controller() {
        ...
        if (self.isCost()) {
            ...
            if (self.inner.match(/\d+D/)) self.cost.debt = parseInt(self.inner.match(/(\d+)D/)[1]);else if (self.inner === "D") self.forceShow = "debt";else if (self.inner === "") self.forceShow = "coin";else self.cost.coin = parseInt(self.inner);
        } else if (self.isVP()) {
            if (self.inner === "") self.forceShow = true;else self.vpAmount = parseInt(self.inner);
        } else {
            self.plainText = self.inner;
        }
    },
    preserveWhitespaces: false,
    template: "<span ng-if=\"$ctrl.plainText.length > 0\">{{$ctrl.plainText}}</span>\n<div\n    class=\"cost-container-wrapper\"\n    ng-if=\"$ctrl.isCost()\"><cost-container\n        coin=\"$ctrl.cost.coin\"\n        potion=\"$ctrl.cost.potion\"\n        debt=\"$ctrl.cost.debt\"\n        force-show=\"$ctrl.forceShow\"></cost-container></div>\n<div class=\"vp-container-wrapper\" ng-if=\"$ctrl.isVP()\"><vp-container amount=\"$ctrl.vpAmount\" force-show=\"$ctrl.forceShow\"></vp-container></div>"
});
